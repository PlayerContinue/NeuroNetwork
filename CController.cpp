...
const int NumNotMineVerts = 4;

const SPoint notMine[NumNotMineVerts] = {
	SPoint(-1, -1),
	SPoint(-1, 1),
	SPoint(1, 1),
	SPoint(1, -1)
...
m_NumNotMines(CParams::iNumNotMines),
m_hwndMain(hwndMain),
m_iGenerations(0),
...
cyClient(CParams::WindowHeight)
{
	...
	//initialize not mines in random position within the application window
	for (int i = 0; i < m_NumNotMines; ++i){
		m_vecNotMines.push_back(SVector2D(RandFloat() * cxClient,
			RandFloat() * cyClient));
	}
	...
	for (int i = 0; i < NumNotMineVerts; ++i){
		m_NotMineVB.push_back(notMine[i]);
	}
...
//	method.
//-------------------------------------------------------------------
void CController::WorldTransform(vector<SPoint> &VBuffer, SVector2D vPos, double size)
{
	//create the world transformation matrix
	C2DMatrix matTransform;

	//scale
	matTransform.Scale(size, size);
...
bool CController::Update()
{
	...
	if (m_iTicks++ < CParams::iNumTicks)
	{
		for (int i = 0; i < m_NumSweepers; ++i)
		{
			...
			if (!m_vecSweepers[i].Update(m_vecMines,m_vecNotMines))
			{
			...
			//see if it's hitting a not_mine
			GrabHit = m_vecSweepers[i].CheckForNotMine(m_vecNotMines,
				CParams::dNotMineScale);

			if (GrabHit >= 0)
			{
				//Since a not mine was hit, we need to decrement the fitness
				m_vecSweepers[i].DecrementFitness();

				//Not found so replace the not mine with another at a random 
				//position
				m_vecNotMines[GrabHit] = SVector2D(RandFloat() * cxClient,
					RandFloat() * cyClient);
			}
...
void CController::Render(HDC surface)
{
	...
	if (!m_bFastRender)
	{
		...
		for (int i = 0; i < m_NumMines; ++i)
		{
			...
			WorldTransform(mineVB, m_vecMines[i],CParams::dMineScale);
		...
		//keep a record of the old pen
		(HPEN)SelectObject(surface, m_RedPen);

		//render the not mine mines
		for (int i = 0; i < m_NumNotMines; ++i)
		{
			//grab the vertices for the mine shape
			vector<SPoint> notmineVB = m_NotMineVB;

			WorldTransform(notmineVB, m_vecNotMines[i], CParams::dNotMineScale);

			//draw the mines
			MoveToEx(surface, (int)notmineVB[0].x, (int)notmineVB[0].y, NULL);

			for (int vert = 1; vert < notmineVB.size(); ++vert)
			{
				LineTo(surface, (int)notmineVB[vert].x, (int)notmineVB[vert].y);
			}

			LineTo(surface, (int)notmineVB[0].x, (int)notmineVB[0].y);
