...
private:
	...
	//index position of closest not_mine
	int m_iClosestNotMine;
...
public:
	...
	bool			Update(vector<SVector2D> &mines, vector<SVector2D> &not_mines);
	...
	SVector2D	GetClosestMine(vector<SVector2D> &objects,int &m_closestObject);

	//checks to see if the minesweeper has 'collected' a mine
	int       CheckForMine(vector<SVector2D> &mines, double size);

	//checks to see if the minesweeper has not come in contact with a mine
	int CheckForNotMine(vector<SVector2D> &mines, double size);
	...
	void			DecrementFitness(){ 

		//Only decrement the fitness if the fitness is greater than zero
		// or the selection type can handle negative values
		//Otherwise we may get too negative an implement
		if (m_dFitness >= 1 || CParams::dSelectionType > 0){
			--m_dFitness;
		}
