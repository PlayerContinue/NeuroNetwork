...
// A vector to the closest not mine (x,y)
//
...
//
//-----------------------------------------------------------------------
bool CMinesweeper::Update(vector<SVector2D> &mines, vector<SVector2D> &not_mines)
{
	...
	SVector2D vClosestMine = GetClosestMine(mines, m_iClosestMine);

	//get the vector for the closest object which is not a mine
	SVector2D vClosestNotMine = GetClosestMine(not_mines, m_iClosestNotMine);

	//normalise it
	Vec2DNormalize(vClosestMine);

	//normalize the non-mine vector
	Vec2DNormalize(vClosestNotMine);
	...
	//add in vector to closest not mine
	inputs.push_back(-1* vClosestNotMine.x);
	inputs.push_back(-1*vClosestNotMine.y);
...
//
//-----------------------------------------------------------------------
SVector2D CMinesweeper::GetClosestMine(vector<SVector2D> &mines, int &m_iClosestObject)
{
...
//----------------------------- CheckForNotMine -----------------------------
//
//  this function checks if the minesweeper has not collides with it's 
// closest mine (calculated earlier and stored in m_iClosestMine)
//-----------------------------------------------------------------------

int CMinesweeper::CheckForNotMine(vector<SVector2D> &mines, double size){
	//Get the distance from the closest mine
	SVector2D DistToObject = m_vPosition - mines[m_iClosestNotMine];

	if (Vec2DLength(DistToObject) < (size + 5))
	{
		return m_iClosestNotMine;
	}
